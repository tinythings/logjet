...
pub fn map_logs(
    ...
    for slice in &db.read_slices()? {
        if let Some(rec) = maybe_slice_to_log(slice, converter) { all.push(rec); }
    }
    for s in &db.read_sched_slices()? {
        if let Some(rec) = maybe_sched_slice_to_log(s, converter) { all.push(rec); }
    }
    for ts in &db.read_thread_states()? {
        if let Some(rec) = maybe_thread_state_to_log(ts, converter) { all.push(rec); }
    }
    for ev in &db.read_ftrace_events()? {
        if let Some(rec) = maybe_ftrace_event_to_log(ev, converter) { all.push(rec); }
    }
    for w in &db.read_spurious_wakeups()? {
        if let Some(rec) = maybe_spurious_wakeup_to_log(w, converter) { all.push(rec); }
    }
    for inst in &db.read_instants()? {
        if let Some(rec) = maybe_instant_to_log(inst, converter) { all.push(rec); }
    }
...
fn maybe_slice_to_log(slice: &PerfettoSlice, converter: &TimestampConverter) -> Option<LogRecord> {
    ...
    Some(LogRecord {
        time_unix_nano: ts, observed_time_unix_nano: ts,
...
fn maybe_thread_state_to_log(ts: &PerfettoThreadState, converter: &TimestampConverter) -> Option<LogRecord> {
    ...
    if let Some(cpu) = ts.cpu { body.push_str(&format!(" cpu={cpu}")); }
    if ts.io_wait == Some(true) { body.push_str(" io_wait"); }
    if let Some(ref blocked) = ts.blocked_function { body.push_str(&format!(" blocked={blocked}")); }
    if let Some(waker) = ts.waker_utid { body.push_str(&format!(" waker={waker}")); }
    ...
    if let Some(cpu) = ts.cpu { attrs.push(KeyValue { key: "perfetto.ts.cpu".to_string(), value: Some(AnyValue { value: Some(Value::IntValue(cpu)) }) }); }
    if let Some(io) = ts.io_wait { attrs.push(KeyValue { key: "perfetto.ts.io_wait".to_string(), value: Some(AnyValue { value: Some(Value::IntValue(i64::from(io))) }) }); }
    if let Some(ref blocked) = ts.blocked_function { attrs.push(KeyValue { key: "perfetto.ts.blocked_function".to_string(), value: Some(AnyValue { value: Some(Value::StringValue(blocked.clone())) }) }); }
...
fn maybe_ftrace_event_to_log(ev: &PerfettoFtraceEvent, converter: &TimestampConverter) -> Option<LogRecord> {
    ...
    if let Some(utid) = ev.utid { attrs.push(KeyValue { key: "perfetto.ftrace.utid".to_string(), value: Some(AnyValue { value: Some(Value::IntValue(utid)) }) }); }
    Some(LogRecord {
        time_unix_nano: t, observed_time_unix_nano: t,
        severity_number: SEVERITY_INFO, severity_text: "INFO".to_string(),
        body: Some(AnyValue { value: Some(Value::StringValue(body)) }),
...
fn maybe_spurious_wakeup_to_log(w: &PerfettoSpuriousWakeup, converter: &TimestampConverter) -> Option<LogRecord> {
    ...
    if let Some(utid) = w.utid { attrs.push(KeyValue { key: "perfetto.sw.utid".to_string(), value: Some(AnyValue { value: Some(Value::IntValue(utid)) }) }); }
    if let Some(waker) = w.waker_utid { attrs.push(KeyValue { key: "perfetto.sw.waker_utid".to_string(), value: Some(AnyValue { value: Some(Value::IntValue(waker)) }) }); }
    Some(LogRecord {
        time_unix_nano: t, observed_time_unix_nano: t,
        severity_number: SEVERITY_INFO, severity_text: "INFO".to_string(),
        body: Some(AnyValue { value: Some(Value::StringValue(body)) }),
...
fn emit_summary(
    ...
    let request = ExportLogsServiceRequest {
        resource_logs: vec![ResourceLogs {
            resource: Some(Resource { attributes: vec![KeyValue { key: "service.name".to_string(), value: Some(AnyValue { value: Some(Value::StringValue("perfetto".to_string())) }) }], dropped_attributes_count: 0, entity_refs: Vec::new() }),
            scope_logs: vec![ScopeLogs { scope: Some(InstrumentationScope { name: "perfetto-ingest".to_string(), version: String::new(), attributes: Vec::new(), dropped_attributes_count: 0 }), log_records: vec![record], schema_url: String::new() }],
            schema_url: String::new(),
        }],
    };
    let payload = request.encode_to_vec();
