...
mod tests {
    ...
    fn int_attr(key: &str, value: i64) -> KeyValue {
        KeyValue {
            key: key.to_string(),
            value: Some(AnyValue { value: Some(Value::IntValue(value)) }),
        }
    }
    ...
    fn record_attribute_string_match() {
        ...
        let payload = make_log_payload(vec![log_record_with_attrs(
            "msg", 13, "WARN",
            &[string_attr("custom.thread", "worker-1")],
        )]);
        assert!(expr.matches_logs_payload(&payload).unwrap());
    }
