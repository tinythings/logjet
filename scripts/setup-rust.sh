...
install_system_pkg() {
    ...
    case "$cmd" in
        ...
        *)
            echo "Could not determine how to install packages on this system." >&2
            return 1 ;;
    esac
}
...
# Partial install — can't recover automatically.
if have_rustc || have_cargo || have_rustup; then
...
echo "Rust installed: rustc $(rustc --version | awk '{print $2}')"
