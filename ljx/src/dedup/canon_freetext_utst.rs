...
fn uuid() {
    assert_eq!(
        classify_token("48574a68-40f7-4b2a-9c3d-1234567890ab"),
        TokenClass::Replace("_UUID_"),
    );
}
...
fn ipv6() {
    assert_eq!(
        classify_token("fd53:7cb8:0383:0000:0000:0000:0000:0001"),
        TokenClass::Replace("_IPV6_"),
    );
}
...
fn ipv6_compressed() {
    assert_eq!(
        classify_token("fd53:7cb8:0383::1:0:0:1"),
        TokenClass::Replace("_IPV6_"),
    );
}
...
fn mac_address() {
    assert_eq!(
        classify_token("fa:00:10:01:86:dd"),
        TokenClass::Replace("_MAC_"),
    );
}
...
fn ipv4() {
    assert_eq!(
        classify_token("192.168.1.1"),
        TokenClass::Replace("_IPV4_"),
    );
}
...
fn hex_with_prefix() {
    assert_eq!(
        classify_token("0x005B4F65"),
        TokenClass::Replace("_HEX_"),
    );
    assert_eq!(
        classify_token("0X1200405"),
        TokenClass::Replace("_HEX_"),
    );
}
...
fn iso8601_timestamp() {
    assert_eq!(
        classify_token("2026-03-07T16:53:03"),
        TokenClass::Replace("_TS_"),
    );
    assert_eq!(
        classify_token("2026-03-07T16:53:03.123Z"),
        TokenClass::Replace("_TS_"),
    );
}
...
fn file_path() {
    assert_eq!(
        classify_token("/data/vendor/pathologyArchive/136/VRAMMON"),
        TokenClass::Replace("_PATH_"),
    );
}
...
fn long_bare_hex() {
    assert_eq!(
        classify_token("48574a68"),
        TokenClass::Replace("_HEX_"),
    );
    assert_eq!(
        classify_token("187cc17b"),
        TokenClass::Replace("_HEX_"),
    );
}
...
fn long_bare_hex_needs_alpha_hex() {
    ...
    assert_eq!(
        classify_token("12345678"),
        TokenClass::Replace("_N_"),
    );
}
...
fn quoted_string_long_replaced() {
    ...
    assert_eq!(
        classify_token("\"abcdefghijklmnopqr\""),
...
fn cpp_symbol_preserved() {
    assert_eq!(
        classify_token("Dispatcher::AreHorizonWarnings"),
        TokenClass::Preserve,
    );
}
...
fn full_sentence_mixed() {
    let input =
        "route progress 192.168.1.1 changed at 2026-03-07T16:53:03 id 48574a68-40f7-4b2a-9c3d-1234567890ab";
    let result = canonicalise_freetext(input);
    assert_eq!(
        result,
        "route progress _IPV4_ changed at _TS_ id _UUID_"
    );
}
